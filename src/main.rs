...
fn get_object_keys(data: &Value, prepend: String) -> Vec<Mutation> {
    ...
    for (key, value) in data.as_object().unwrap() {
        ...
        if value.is_string() {
            current_scope_values.push(Mutation { key, value: parse_string(value)})
            ...
            for line in value.as_array().unwrap() {
                for (key_j, value) in line.as_object().unwrap() {
                    ...
                    current_scope_values.push(Mutation { key:  array_key, value: parse_string(value) });
                }
                index += 1;
            }
        }
    ...
    for mutation in data {
        ...
        match keys.iter().position(|&x| x == "$") {
                ...
                match current_map.get(&*json_key) {
                    Some(value) => {
                        ...
                        if current_items.len() > index {
                            ...
                            merge(&mut found_object, json!({last_array_key.to_string(): parse_string(&Value::String(mutation.value))}));
                            ...
                            if current_items.len() > 0 {
                                ...
                                current_items[index] = json!({last_array_key.to_string(): parse_string(&Value::String(mutation.value))});
                            } else {
                                for i in 0..(index + 1) {
                                    current_items.push(json!({}));
                                }
            ...
            None => {
                ...
                current_map.insert(last_key.to_string(), Value::String(parse_string(&Value::String(mutation.value))));
            },
        };
